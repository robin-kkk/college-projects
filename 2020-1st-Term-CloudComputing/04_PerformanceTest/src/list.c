...
void init_btree_head(struct B_TREE_HEAD_T *head, int degree) {
    ...
    head->leaf = TRUE;
    head->parent = 0;
    head->childs = (struct B_TREE_HEAD_T**) calloc(degree+1, sizeof(struct B_TREE_HEAD_T*));
    if (head->childs == 0) {
        perror("[ERROR] init_btree_head()");
        exit(-1);
    }
}
                  ...
                  struct B_TREE_T *tree)
{
    ...
    if (p_head->parent == 0) { // 루트노드인 경우
        ...
        if (left == 0 || right == 0) {
            perror("[ERROR] btree_divide(): left or right not allocated\n");
            exit(-1);
        }
        ...
        if (left->elements == 0) {
            perror("[ERROR] btree_divide(): left->elements not allocated\n");
            exit(-1);
        }
    ...
    right = (struct B_TREE_NODE_T*) malloc(sizeof(struct B_TREE_NODE_T));
    if (right == 0) {
        perror("[ERROR] btree_divide(): right not allocated\n");
        exit(-1);
    }
    ...
    right->elements = (struct LIST_HEAD_T*) malloc(sizeof(struct LIST_HEAD_T));
    if (right->elements == 0) {
        perror("[ERROR] btree_divide(): right->elements not allocated\n");
        exit(-1);
    }
    init_list_head(right->elements);
    right->elements->next = target->next;
    ...
    if (parent_el_idx == p_head->parent->n) {
        ...
        target->next = pr_el_head->next;
        target->prev = pr_el_head;
        if (pr_el_head->next->prev != 0) pr_el_head->next->prev = target;
        pr_el_head->next = target;
    }
                                            ...
                                            int direction)
{
    ...
    if (direction == 0) { // 왼쪽 서브 트리에 포함되는 경우
        ...
        left = list_entry(subtree->head.childs[0], struct B_TREE_NODE_T, head);
        right = list_entry(subtree->head.childs[1], struct B_TREE_NODE_T, head);
        ...
        if (alternative == 0) {
            ...
            if (p_node->head.n >= t) {
                if (el_idx == 0) {
                    ...
                    left = list_entry(p_node->head.childs[el_idx-1], struct B_TREE_NODE_T, head);
                    right = list_entry(p_node->head.childs[el_idx], struct B_TREE_NODE_T, head);
                    merged = _merge_with_parent(left, right, el_head->prev);
                    p_node->head.childs[el_idx-1] = &merged->head;
                    alternative = _get_predecessor(merged, tree);
